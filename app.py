...
if st.session_state.current_page == "航线规划":
    ...
    with col_map:
        ...
        st.caption("🟥 左侧绕行(红平滑曲线) | 🟧 右侧绕行(橙平滑曲线) | 🟦 最优航线(蓝加粗曲线) | ⚫直接飞越(灰曲线)")
        ...
        def render_map():
            ...
            if "左侧绕行" in st.session_state.all_routes:
                folium.PolyLine(st.session_state.all_routes["左侧绕行"], color="#FF0000", weight=5, opacity=0.8, popup="左侧绕行平滑曲线").add_to(m)
            if "右侧绕行" in st.session_state.all_routes:
                folium.PolyLine(st.session_state.all_routes["右侧绕行"], color="#FFA500", weight=5, opacity=0.8, popup="右侧绕行平滑曲线").add_to(m)
