...
# 终极强制汉化：覆盖缩放+全部绘图按钮，彻底消除所有英文tooltip
st.markdown('''
<style>
/* 1. 彻底隐藏所有原生英文悬浮提示 */
.leaflet-tooltip,
...
if st.session_state.current_page == "航线规划":
            ...
            for idx in range(len(st.session_state.obstacle_polygons)):
                with st.expander(f"障碍物 {idx+1}"):
                    st.session_state.obstacle_heights[idx] = st.slider(
                        "障碍物高度(米)",1,200,
                        st.session_state.obstacle_heights.get(idx,50),
                        key=f"h_{idx}"
                    )
                    if st.button("删除障碍物",key=f"del_{idx}"):
    ...
    with col_map:
        ...
        def render_map():
            ...
            m = folium.Map([center_lat,center_lon],zoom_start=17,
            ...
            folium.PolyLine(st.session_state.current_route_points,color="blue",weight=4,popup="当前规划航线").add_to(m)

            for idx,poly in enumerate(st.session_state.obstacle_polygons):
                folium.Polygon(poly,color="red",fill=True,fill_color="red",fill_opacity=0.4,
                    popup=f"障碍物 | 高度：{st.session_state.obstacle_heights.get(idx,50)}米").add_to(m)
...
# ========================= 飞行监控页面 =========================
    ...
    while st.session_state.is_running:
        ...
        chart_area.line_chart(st.session_state.df_history,x="时间",y="序号")
        list_area.dataframe(st.session_state.df_history.tail(10),hide_index=True)
